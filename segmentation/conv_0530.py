...
def fix_mask_dimensions(mask, features):     
    ...
    if mask.size(2)!=goal_height:
        ...
        zeros = torch.zeros((batch_size, 1, add_dimensions, mask.size(3)), dtype=mask.dtype, device=mask.device)
        mask = torch.cat((mask, zeros), dim=2)
    if mask.size(3)!=goal_width:
        add_dimensions = goal_width - mask.size(3)
        zeros = torch.zeros((batch_size, 1, mask.size(2), add_dimensions), dtype=mask.dtype, device=mask.device)
...
def train_model(model, dataloader, criterion, optimizer, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in dataloader:
            ...
            # ignore_mask = create_ignore_mask(ignore_coords, masks)
...
def calculate_loss(criterion, outputs, mask, ignore_mask):
    ...
    save_image(logits_mask[0].squeeze().cpu(), 'output.png')
    save_image(mask[0].squeeze(0).cpu(), 'output_mask.png')
    save_image(ignore_mask[0].squeeze(0).cpu(), 'output_ignore.png')
...
def create_ignore_mask(ignore_path, masks_raw):
    with open(ignore_path[0], 'rb') as f:
        ignore_coords = list(set(pickle.load(f)))
    ignore_mask = torch.zeros_like(masks_raw).squeeze().squeeze() #CHANGED
...
def visualize_predictions(model, dataloader, threshold=.5):
    model.eval()  # Set the model to evaluation mode
    with torch.no_grad():  # Do not calculate gradients
        for b, batch in enumerate(dataloader):
            features_raw = batch['features'].squeeze()
            masks_raw = batch['mask'].squeeze().squeeze()
            masks_raw = fix_mask_dimensions(masks_raw, features_raw)
            image_predicted=model(features_raw.unsqueeze(0).float())
            ...
            padding = np.abs(images[0].shape[0] - preds.shape[2])
            ...
            concatenated.save(f'/pasteur/u/aunell/cryoViT/segmentation/results0531/Conv1layer_{b}.png')
...
class MyModel(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1536, 1, kernel_size=3, stride=1, padding=1) #add conv2dtranpose(kernel size 1 stride 14) can do upsampling as well
        self.upsample1 = nn.Upsample(scale_factor=14, mode='bilinear', align_corners=True)
        
        # self.conv2 = nn.Conv2d(512, 1, kernel_size=3, stride=1, padding=1)
        # self.upsample2 = nn.Upsample(scale_factor=7, mode='bilinear', align_corners=True)
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load('/pasteur/u/aunell/cryoViT/segmentation/conv_model.pth'))
    # train_model(model, dataloader, criterion, optimizer, num_epochs=30)
    # torch.save(model.state_dict(), '/pasteur/u/aunell/cryoViT/segmentation/conv_model.pth')
    ...
    visualize_predictions(model, test_dataloader, threshold=0.50)